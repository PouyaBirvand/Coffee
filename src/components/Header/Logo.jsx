...
function ProductTitle() {
  ...
  return (
    <div className="relative text-center mt-2 h-[2rem]">
      <h1
        className={`inline-block font-extrabold text-deep-mahogany px-4 leading-[2rem] relative text-[2.6rem] sm:w-40 lg:text-[2rem] ${
          isExpanded &&
          'lg:text-[3.2rem] -mt-2 text-[3.4rem] leading-[3.5rem] sm:mt-3 md:!text-[2.7rem]'
        } ${
          isCartPage
            ? '!text-[2rem] relative bottom-[2.5rem] sm:bottom-[2rem] md:!text-[1.7rem] leading-[2.5rem]'
            : ''
        }`}
      >
        ...
        {isExpanded ? (
          ...
          <motion.div whileHover={{ scale: 1.02 }}>کافه کهن</motion.div>
        )}
