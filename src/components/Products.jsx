...
const createCoffeeItems = (category) => {
  const items = Array.from({ length: 35 }, (_, index) => ({
    id: `${category}-${index + 1}`,
    image: `/assets/images/Coffee${index + 1}.png`,
    title: category,
    price: index < 3 ? [4.99, 5.99, 3.99][index] : 6.99,
    discount: index < 3 ? [0.05, 0.03, 0.02][index] : 0,
    fullText: 'Our signature espresso is a rich blend of premium coffee beans, roasted to perfection for a bold and intense flavor.'
  }));
...
useEffect(() => {
  setItems(createCoffeeItems(category));
}, [category]);
      ...
      <div className={`relative ${!isExpanded ? 'shadow-md bg-[#835a36] bg-opacity-50 rounded-2xl p-5' : ''} mx-auto`}>
        <div className="relative -top-8">
          <motion.img
            src={item.image}
            alt={item.title}
            className={`m-auto max-h-[12.5rem] sm:max-h-[10rem] md:max-h-[10rem] !scale-[1.3]  object-contain ${isExpanded ? ' fixed left-0 right-0 !scale-[1.9] translate-y-[3rem] absloute w-[82%]' : ''}`}
