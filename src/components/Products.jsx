...
function Products({ category, isExpanded }) {
  ...
  const renderSlide = (item) => (
    <SwiperSlide key={item.id} className={`pt-14 z-0 ${isExpanded ? 'pt-[5.9rem] sm:pt-[8rem]' : ''}`}>
      <div className={`relative ${!isExpanded ? 'shadow-md bg-[#835a36] bg-opacity-50 rounded-2xl p-4' : ''} mx-auto`}>
        <div className="relative -top-8">
          <motion.img
            src={item.image}
            alt={item.title}
            className={`m-auto max-h-[12.5rem] max-w-[100%]  md:max-h-[10rem] !scale-[1.3]  object-contain ${isExpanded ? ' fixed left-0 right-0 !scale-[1.9] translate-y-[3rem] absloute w-[82%]' : ''}`}
